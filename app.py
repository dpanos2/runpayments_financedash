...
# ── Debug: raw QBO response ───────────────────────────────────────────────────

@app.route('/debug/raw')
def debug_raw():
    """Return raw QBO P&L JSON — use to diagnose processor mapping issues."""
    ...
    tokens = store.get_tokens()
    if not tokens:
        return jsonify({'error': 'Not connected to QuickBooks'}), 400
...
def debug_processed():
    """Return the processed data as stored — use to verify parsing."""
    secret = request.args.get('secret', '')
    if secret != os.environ.get('REFRESH_SECRET', ''):
        return jsonify({'error': 'Unauthorized'}), 401
    return jsonify(store.get_data())
...
def _refresh_data():
    ...
    try:
        ...
        # Log column structure to help diagnose mapping issues
        columns = report.get('Columns', {}).get('Column', [])
        print(f'[refresh] Columns: {[(i, c.get("ColTitle"), c.get("ColType")) for i, c in enumerate(columns)]}')

        # Log top-level row labels
        ...
        for row in rows:
            if row.get('type') == 'Section':
                ...
                if col_data:
                    print(f'[refresh] Section total label: "{col_data[0].get("value", "")}"')
                for sub in row.get('Rows', {}).get('Row', []):
                    sub_data = sub.get('ColData', [])
                    if sub_data:
                        print(f'[refresh]   Account: "{sub_data[0].get("value", "")}"')
        ...
        if data:
            sample = data[0]
            print(f'[refresh] Sample month: {sample.get("month")} | income={sample.get("totalIncome")} | netIncome={sample.get("netIncome")}')
    except Exception as e:
        import traceback
